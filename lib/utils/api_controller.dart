...
class APICalls {
  ...
  void initialize(String userId, String accessToken, String refreshToken, bool keepLoginInPreferences) async {
    _USER_ID = userId;
  ...
  void tryInitializeFromPreferences() async {
    ...
    if (refresh_prefs == null) _redirectToLogin();
    else {
  ...
  void getItem(String endpoint, List<String> pathParams, Function onSuccess, Function onError) async {
    ...
    if (response.statusCode == _UNAUTHORIZED) {
      _refresh(() => getItem(endpoint, pathParams, onSuccess, onError), () => _redirectToLogin());
    } else if (response.statusCode ~/ 100 == 2) {
      onSuccess(jsonDecode(response.body));
    } else {
      String errorMessage = 'No error message provided';
      if (jsonDecode(response.body).containsKey('error_message')) errorMessage = jsonDecode(response.body)['error_message'];
  ...
  void getCollection(String endpoint, List<String> pathParams, Map<String, String> queryParams, Function onSuccess, Function onError) async {
    ...
    if (response.statusCode == _UNAUTHORIZED) {
      _refresh(() => getCollection(endpoint, pathParams, queryParams, onSuccess, onError), () => _redirectToLogin());
    } else if (response.statusCode ~/ 100 == 2) {
      onSuccess(jsonDecode(response.body));
    } else {
      String errorMessage = 'No error message provided';
  ...
  void postItem(String endpoint, List<String> pathParams, Map<String, dynamic> bodyData, Function onSuccess, Function onError) async {
    ...
    if (response.statusCode == _UNAUTHORIZED) {
      _refresh(() => postItem(endpoint, pathParams, bodyData, onSuccess, onError), () => _redirectToLogin());
    } else if (response.statusCode ~/ 100 == 2) {
      onSuccess(jsonDecode(response.body));
    } else {
      String errorMessage = 'No error message provided';
  ...
  void putItem(String endpoint, List<String> pathParams, Map<String, dynamic> bodyData, Function onSuccess, Function onError) async {
    ...
    if (response.statusCode == _UNAUTHORIZED) {
      _refresh(() => putItem(endpoint, pathParams, bodyData, onSuccess, onError), () => _redirectToLogin());
    } else if (response.statusCode ~/ 100 == 2) {
      onSuccess(jsonDecode(response.body));
    } else {
      String errorMessage = 'No error message provided';
  ...
  void deleteItem(String endpoint, List<String> pathParams, Function onSuccess, Function onError) async {
    ...
    if (response.statusCode == _UNAUTHORIZED) {
      _refresh(() => deleteItem(endpoint, pathParams, onSuccess, onError), () => _redirectToLogin());
    } else if (response.statusCode ~/ 100 == 2) {
      onSuccess(jsonDecode(response.body));
    } else {
      String errorMessage = 'No error message provided';
  ...
  Uri _buildUri(String endpoint, List<String> pathParams, Map<String, String> queryParams) {
